...
(function(){
  ...
  // Tracks only whether a moderation session exists. The signed-in identity is
  // deliberately never read into the page, so it cannot end up on screen.
  ...
  function renderComments(list){
    ...
    // Deliberately does not name the signed-in account: the banner is often on
    // screen while sharing or screenshotting, and the identity adds nothing.
    const adminBarHtml = adminSession
      ? '<div class="admin-bar" id="admin-bar"><span class="admin-badge">Moderating</span>'+
          '<span class="admin-note">Delete controls are shown on each comment.</span>'+
        '</div>'
      : '';
          ...
          return '<div class="comment-item" data-id="'+c.id+'">'+
            ...
            '<div class="comment-body-col">'+
              '<div class="comment-meta"><span class="comment-author">'+escapeHtml(c.author)+'</span><span class="comment-time">'+timeAgo(c.created_at)+'</span>'+
                (adminSession ? '<button class="comment-delete" data-del="'+c.id+'" aria-label="Delete comment">Delete</button>' : '')+
              '</div>'+
      ...
      btn.addEventListener('click', async ()=>{
        ...
        try{
          ...
          if(!res.ok){
            ...
            if(res.status===401){ adminSession=null; }
 ...
 * ASSUMPTIONS PANEL
 *
 * Presents the structural forecast drivers from assumptions.js as a labelled,
 * editable grid. Edits mutate FIXED in place and re-run the model immediately.
 *
 * Reported actuals are deliberately excluded — they are shown read-only in the
 * tables above and should only change when SpaceX publishes or restates.
 *
 * Running locally, "Save to assumptions.js" writes the file through the dev
 * server. On the deployed site there is no such endpoint, so the panel offers
 * Copy/Download instead and a visitor's edits stay in their browser session.
 * ==========================================================================*/
(function(){
  const QTR = 'quarterly';   // 10 values, Q326 -> Q428
  const ONE = 'scalar';

  // kind: how the stored value is shown. pct is stored as a fraction.
  const GROUPS = [
    { title:'Space segment', rows:[
      { path:'space.revGrowth', shape:QTR, kind:'pct', label:'Revenue growth, QoQ' },
      { path:'space.cogsPct',   shape:QTR, kind:'pct', label:'Cost of revenue, % of revenue' },
      { path:'space.rndPct',    shape:QTR, kind:'pct', label:'R&D, % of revenue' },
      { path:'space.sgaPct',    shape:QTR, kind:'pct', label:'SG&A, % of revenue' },
      { path:'space.capexPct',  shape:QTR, kind:'pct', label:'Capex, % of revenue' },
    ]},
    { title:'Connectivity segment', rows:[
      { path:'connectivity.subAdds',      shape:QTR, kind:'num', label:'Net subscriber adds', unit:'mm' },
      { path:'connectivity.arpu',         shape:QTR, kind:'num', label:'Consumer ARPU', unit:'$/mo' },
      { path:'connectivity.entGovGrowth', shape:QTR, kind:'pct', label:'Enterprise & gov growth, QoQ' },
      { path:'connectivity.cogsPct',      shape:QTR, kind:'pct', label:'Cost of revenue, % of revenue' },
      { path:'connectivity.rndPct',       shape:QTR, kind:'pct', label:'R&D, % of revenue' },
      { path:'connectivity.sgaPct',       shape:QTR, kind:'pct', label:'SG&A, % of revenue' },
      { path:'connectivity.capexPct',     shape:QTR, kind:'pct', label:'Capex, % of revenue' },
    ]},
    { title:'AI segment', rows:[
      { path:'ai.monetizable', shape:QTR, kind:'pct', label:'Monetizable share of compute' },
      { path:'ai.cogsPct',     shape:QTR, kind:'pct', label:'Cost of revenue, % of revenue' },
      { path:'ai.rndPct',      shape:QTR, kind:'pct', label:'R&D, % of revenue' },
      { path:'ai.sgaPct',      shape:QTR, kind:'pct', label:'SG&A, % of revenue' },
      { path:'ai.grok',        shape:QTR, kind:'num', label:'Grok subscription revenue', unit:'$mm/qtr' },
      { path:'ai.cursor',      shape:QTR, kind:'num', label:'Cursor revenue', unit:'$mm/qtr' },
      { path:'ai.adGrowth',    shape:QTR, kind:'pct', label:'Advertising growth, QoQ' },
    ]},
    { title:'Below the line', rows:[
      { path:'financing.otherIncome', shape:QTR, kind:'num', label:'Other income (expense), net', unit:'$mm/qtr' },
      { path:'financing.tax',         shape:QTR, kind:'num', label:'Income tax', unit:'$mm/qtr' },
    ]},
    { title:'Financing & depreciation', rows:[
      { path:'financing.itShare',       shape:ONE, kind:'pct', label:'IT equipment share of capex' },
      { path:'financing.itLife',        shape:ONE, kind:'num', label:'IT equipment life', unit:'quarters' },
      { path:'financing.facilityLife',  shape:ONE, kind:'num', label:'Facility life', unit:'quarters' },
      { path:'financing.legacyAddback', shape:ONE, kind:'num', label:'Legacy non-cash add-back', unit:'$mm/qtr' },
      { path:'financing.existingDebt',  shape:ONE, kind:'num', label:'Existing debt', unit:'$mm' },
      { path:'financing.existingRate',  shape:ONE, kind:'pct', label:'Rate on existing debt', unit:'/yr' },
  ...
  const getRef = (p) => p.split('.').reduce((o,k)=>o[k], FIXED);
  const getParent = (p) => { const k=p.split('.'); return [k.slice(0,-1).reduce((o,x)=>o[x],FIXED), k[k.length-1]]; };
  const toDisplay = (v,kind) => kind==='pct' ? +(v*100).toFixed(4) : v;
  const toStored  = (v,kind) => kind==='pct' ? v/100 : v;

  let dirty = false;

  function cellInput(value, kind, onChange){
    const inp = document.createElement('input');
    inp.type='number'; inp.className='assump-input';
    inp.step = kind==='pct' ? '0.1' : 'any';
    inp.value = toDisplay(value, kind);
    inp.addEventListener('input', ()=>{
      const raw = parseFloat(inp.value);
      if(Number.isNaN(raw)) return;
      onChange(toStored(raw, kind));
      dirty = true;
      updateDirty();
      renderAll();
    });
  ...
  function updateDirty(){
    const b = document.getElementById('assump-dirty');
    if(b) b.textContent = dirty ? 'Unsaved changes' : '';
  }

  function buildPanel(){
    const host = document.getElementById('assumptions-body');
    if(!host) return;
    host.innerHTML='';

    GROUPS.forEach(g=>{
      const sec = document.createElement('div'); sec.className='assump-group';
      const h = document.createElement('div'); h.className='assump-group-title'; h.textContent=g.title;
      sec.appendChild(h);
      ...
      if(g.rows.some(r=>r.shape===QTR)){
        ...
        tr.appendChild(Object.assign(document.createElement('th'),{className:'assump-rowlabel',textContent:'Driver'}));
        Q.forEach(q=>tr.appendChild(Object.assign(document.createElement('th'),{textContent:q})));
      ...
      g.rows.forEach(row=>{
        const tr=document.createElement('tr');
        const label=document.createElement('td'); label.className='assump-rowlabel';
        label.textContent = row.label + (row.kind==='pct' ? ' (%)' : (row.unit ? ' ('+row.unit+')' : ''));
        tr.appendChild(label);

        if(row.shape===QTR){
          const arr = getRef(row.path);
          arr.forEach((v,i)=>{
            const td=document.createElement('td');
            td.appendChild(cellInput(v, row.kind, (nv)=>{ arr[i]=nv; }));
            tr.appendChild(td);
          });
        } else {
          const [parent,key]=getParent(row.path);
  ...
  function serialize(){
    ...
    const obj = (o, indent)=>{
      ...
      const parts = Object.entries(o).map(([k,v])=>{
        if(Array.isArray(v)) return padIn+k+': '+(typeof v[0]==='string' ? JSON.stringify(v) : arr(v));
        if(v && typeof v==='object') return padIn+k+': '+obj(v, indent+2);
 ...
 * ==========================================================================*/`;
  ...
  function download(){
    ...
    a.href = URL.createObjectURL(blob); a.download='assumptions.js';
  ...
  async function copy(){
    const status = document.getElementById('assump-status');
    try{ await navigator.clipboard.writeText(serialize()); status.textContent='Copied assumptions.js to clipboard'; }
    catch{ status.textContent='Clipboard blocked — use Download instead'; }
  }
